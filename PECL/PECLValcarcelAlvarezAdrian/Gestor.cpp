...
bool Gestor::buscarNumero(int numero){
...
int Gestor::randomIDestandar(){
    srand(static_cast<unsigned>(time(nullptr)));
	int limite_inferior = 1;
	int limite_superior = 49;
	int randomID;
	do{
        randomID = limite_inferior+rand()%(limite_superior+1-limite_inferior);
	}while (buscarNumero(randomID));
	return randomID;
}
int Gestor::randomIDurgente(){
    srand(static_cast<unsigned>(time(nullptr)));
	int limite_inferior = 51;
...
int Gestor::randomNumSegestandar(){
    srand(static_cast<unsigned>(time(nullptr)));
	int limite_inferior = 1;
	int limite_superior = 499;
	int randomID;
	do{
        randomID = limite_inferior+rand()%(limite_superior+1-limite_inferior);
	}while (buscarNumero(randomID));
...
int Gestor::randomNumSegurgente(){
    srand(static_cast<unsigned>(time(nullptr)));
	int limite_inferior = 501;
...
void Gestor::encolarPedidos()
{
    ...
    while(!pila.estaVacia())
    {
        if(!pila.tipoDato())
        {
            ...
            {
                pila.setPrioridad(randomIDestandar());//ajuste);
                pila.setNumseg(randomNumSegestandar());
                colaA.encolar(pila.obtenerPedido());
                pila.desapilar();
            }
            else
            {
                pila.setPrioridad(randomIDestandar());
                pila.setNumseg(randomNumSegestandar());
        ...
        else
        {
            ...
            {
                pila.setPrioridad(randomIDurgente());
                pila.setNumseg(randomNumSegurgente());
